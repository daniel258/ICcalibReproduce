...
## This script reproduce the results in Table A.1 in the Supplementary Materials
...
# Check IterSimCox.R to see how each column was calculated
...
#### Summarize Results #####
# Calculate estimated standard errors from estimated variances
...
# Summarize separately for mean, se, se.hat and ci
#  Use "melt()" to change the data format from wide to long
melted.est <- all.results.df %>% select(beta0, n.points, lvcf.beta, midI.beta, cox.beta, cox.rs.beta) %>%  melt(id.vars = c("beta0","n.points"))
...
# Put everthing togethe
final <- cbind(est, SE = se$se, CP95 = ci$cp95)
levels(final$Method) <- c("LVCF", "MidI", "PH-OC", "PH-RSC")
