...
seed <- my.seeds[which(as.numeric(rownames(my.seeds))==n.points), which(as.numeric(colnames(my.seeds))==exp(beta0))] + 10
set.seed(seed)
...
stopCluster(cl)
save.image(paste0("CPBSinglePexpBeta",exp(beta0),"W",n.points,"N",n.sample, "key", Sys.getpid(), ".RData"))
write.table(Results, paste0("CPBSinglePexpBeta",exp(beta0),"W",n.points,"N",n.sample, "key", Sys.getpid(),".txt"))
sessionInfo()
